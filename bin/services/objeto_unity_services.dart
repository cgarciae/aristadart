...
class ObjetoUnityServices extends AristaService<ObjetoUnity> {
  ...
  @app.Route('/:id/userFile',
      methods: const [app.POST, app.PUT], allowMultipartRequest: true)
  @Private()
  @Encode()
  Future<ObjetoUnity> NewOrUpdateUserFile(String id,
    ...
    ObjetoUnity delta = new ObjetoUnity()
      ..updatePending = true
      ..userFile = (new FileDb()..id = userFile.id);
  ...
  Future<ObjetoUnity> Publish(String id) async {
    ...
    if (!(obj.active && obj.updated)) throw new app.ErrorResponse(400,
        "No se puede publicar ObjetoUnity. Estado Actual: ${encodeJson(obj)}");
  ...
  @Private()
  @Encode()
  Future<ObjetoUnity> UpdateModels(
    ...
    if (form.ios != null &&
        form.ios is app.HttpBodyFileUpload &&
    ...
    if (form.android != null &&
        form.android is app.HttpBodyFileUpload &&
        form.android.content is List &&
        form.android.content.isNotEmpty) {
      print("android");
      ...
      //Guardar unicamente el [id] en delta
      delta
        ..android = (new FileDb()..id = newFile.id)
        ..androidUpdated = true;
    }

    if (form.windows != null &&
        form.windows is app.HttpBodyFileUpload &&
        form.windows.content is List &&
        form.windows.content.isNotEmpty) {
      print("windows");
      ...
      //Guardar unicamente el [id] en delta
      delta
        ..windows = (new FileDb()..id = newFile.id)
        ..windowsUpdated = true;
    }

    if (form.osx != null &&
        form.osx is app.HttpBodyFileUpload &&
        form.osx.content is List &&
        form.osx.content.isNotEmpty) {
      print("osx");
  ...
  @Private(ADMIN)
  @Encode()
  Future<List<ObjetoUnity>> Find(@app.QueryParam() bool updatePending,
      @app.QueryParam() String userId, @app.QueryParam() bool active,
  ...
  Future<List<ObjetoUnity>> All(@app.QueryParam() bool updatePending,
      @app.QueryParam() bool active, @app.QueryParam() bool public) async {
  ...
  Future<FileDb> ActualizarModelo(FileDb modelo, String system,
