...
testAuthorization (@Authorization String auth) => auth;
...
@Encode()
testPoly() {
  var inst = new Const2()..id = "some id";

  return inst;
}

@app.Route('/testOptional/:mandatory')
testOptional(String mandatory, {@app.QueryParam() String optional}) =>
    "$mandatory $optional";

@app.Route('/testQuery')
testQuery(@app.QueryParam('algo') String algo,
    @app.QueryParam('mas') String mas) => "$algo $mas";

@app.Route('/testVuforiaImage',
...
@app.Route('/updateVuforiaImage/:id',
    methods: const [app.PUT], allowMultipartRequest: true)
@Encode()
Future<VuforiaResponse> updateVuforiaImage(
    String id, @app.Body(app.FORM) Map form) async {
  HttpBodyFileUpload file = FormToFileUpload(form);
...
testOverride() async {
  ...
  var delta = new VuforiaResponse()
    ..target_record = (new VuforiaTargetRecord()..width = 2);

  await db.update(col, where.id(StringToId(id)), delta, override: false);
...
testError2(int n) async {
  ...
  if (n != 1) throw new Exception("invalid id");
...
gridFSTest(@app.Attr() MongoDb dbConn) {
  ...
  return gridIn
      ...
      .then((GridOut gridOut) =>
          gridOut.writeToFilename('../web/test/file_out.txt'))
      .then((_) => new IdResp()..id = gridIn.id.toHexString());
}
...
testEsteban(@app.Attr() MongoDb dbConn, @Decode() User usuario) async {
  ...
  return new IdResp()..id = usuario.id;
}
